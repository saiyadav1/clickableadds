  ...
  return (
    <aside className="space-y-6 sticky top-32">
      ...
      <div className="p-2 bg-slate-50 rounded-2xl border border-slate-100 flex items-center">
        ...
        />
        <button className="bg-[#05003b] text-white p-3 rounded-xl hover:bg-slate-600 transition-colors">
      ...
      <div className="bg-white border border-slate-100 rounded-[2rem] p-8 shadow-sm">
        <h4 className="text-[#05003b] font-black mb-6 flex items-center gap-2">
           <span className="w-1.5 h-4 bg-slate-600 rounded-full" />
           Categories
        </h4>
        <div className="space-y-3">
          {["Market Trends", "SEO Strategy", "Tech News", "Growth"].map(item => (
            <div key={item} className="flex justify-between items-center p-3 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer group">
              <span className="text-sm font-bold text-slate-600 group-hover:text-slate-600">{item}</span>
              <span className="text-[10px] font-black text-slate-400 bg-slate-100 px-2 py-1 rounded-md group-hover:bg-slate-100 group-hover:text-slate-600">12</span>
            </div>
          ))}
        </div>
      </div>
