...
export default function ServicesSection() {
  ...
  return (
    <section className="relative bg-[#F8FAFC] py-10 overflow-hidden">
      ...
      <div className="absolute top-0 left-1/4 w-96 h-96 bg-slate-100/50 rounded-full blur-[120px] -translate-y-1/2 pointer-events-none" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {servicesData.map((service) => (
            ...
            >
              ...
              <Link
                href={`/services/${service.slug}`}
                className={`group/btn inline-flex items-center gap-2 text-sm font-bold tracking-tight transition-colors duration-300 ${service.color}`}
              >
                <span className="relative py-1 text-primary transition-colors duration-300">
                  Explore Service
                  <span className="absolute bottom-0 text-primary left-0 w-0 h-[2px] bg-current transition-all duration-300 group-hover/btn:w-full" />
                </span>
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-200 text-primary transition-colors duration-300 group-hover/btn:bg-slate-300 group-hover/btn:text-slate-900">
                  <svg
